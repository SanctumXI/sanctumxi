-----------------------------------
-- Sanctum combat rework: direct-damage spell calculations
...
xi.spells.damage.useDamageSpell = function(caster, target, spell)
    ...
    local resolveStacks = 0
    local consumesCascade =
        skillType == xi.skill.ELEMENTAL_MAGIC and
        spellId ~= xi.magic.spell.DEATH and
        caster:hasStatusEffect(xi.effect.CASCADE)

    if spellId == xi.magic.spell.HOLY or spellId == xi.magic.spell.HOLY_II then
        resolveStacks = xi.job_utils.paladin.consumeResolve(caster)
    end
    ...
    finalDamage = xi.job_utils.paladin.applyResolveDamageBonus(finalDamage, resolveStacks, 20)
...
local sanctumModule = Module:new('sanctum_combat_damage_spell_calculations')
