-----------------------------------
-- Sanctum combat rework: shared weapon-skill calculations
...
local function getSingleHitDamage(attacker, target, dmg, ftp, wsParams, calcParams)
    ...
    if xi.combat.physical.isBlocked(target, attacker) then
        ...
        xi.job_utils.paladin.addResolveStack(target)
    end
...
xi.weaponskills.doRangedWeaponskill = function(attacker, target, wsID, wsParams, tp, action, primaryMsg)
    ...
    if
        attacker:getObjType() == xi.objType.PC and
        (blastArrowEmpowered or blastShotEmpowered)
    then
...
xi.weaponskills.takeWeaponskillDamage = function(defender, attacker, wsParams, primaryMsg, attack, wsResults, action)
    ...
    if wsResults.wsID and wsResults.wsID ~= 0 then
        local resolveStacks = xi.job_utils.paladin.consumeResolve(attacker)

        finaldmg = xi.job_utils.paladin.applyResolveDamageBonus(finaldmg, resolveStacks, 5)
    end
...
local sanctumModule = Module:new('sanctum_combat_shared_calculations')
