-- Sanctum: restore model sizes on pools that shipped with NULL.
-- A NULL modelHitboxSize leaves the mob a zero hitbox, and melee range is
-- hitbox + 2 + the target's hitbox, so those mobs and pets reach less far than
-- every sibling of the same type. Four jug pets hit this during the BST pass
-- (Nursery Nazuna, Flytrap Familiar, Crafty Clyvonne, Shellbuster Orob); this
-- covers the rest of the table.
--
-- Values are the most common size/hitbox pair among non-NULL pools of the same
-- family, falling back to the ecosystem's most common pair where a family has
-- no sized sibling. Generated, not hand-written.
