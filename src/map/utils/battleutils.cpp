...
{
    const auto spikeElement = static_cast<ELEMENT>(static_cast<uint8>(GetSpikesDamageType(Action->spikesEffect)) - (uint8)xi::DamageType::Elemental);
...
void HandleEnspell(CBattleEntity* PAttacker, CBattleEntity* PDefender, action_result_t* Action, bool isFirstSwing, CItemWeapon* weapon, int32 finaldamage, CAttack& attack)
{
    ...
    if (PAttacker->StatusEffectContainer->HasStatusEffect(EFFECT_DRAIN_SAMBA) && PDefender->m_EcoSystem != xi::Ecosystem::Undead)
    {
    ...
    if (previous_daze != EFFECT_NONE)
    {
        ...
        {
            if (previous_daze == EFFECT_DRAIN_DAZE && PDefender->m_EcoSystem != xi::Ecosystem::Undead)
            {
...
auto TakePhysicalDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, PHYSICAL_ATTACK_TYPE physicalAttackType, int32 damage, bool isBlocked, uint8 slot, uint16 tpMultiplier, CBattleEntity* taChar, bool giveTPtoVictim, bool giveTPtoAttacker, bool isCounter, bool isCovered, CBattleEntity* POriginalTarget) -> int32
{
    ...
    if (damage > 0)
    {
        ...
        else
        {
            ...
            if (sub_weapon && sub_weapon->getDmgType() > xi::DamageType::None && sub_weapon->getDmgType() < xi::DamageType::HandToHand &&
...
auto TakeSkillchainDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, int32 lastSkillDamage, CBattleEntity* taChar) -> int32
{
    ...
    // Skillchain Damage EXP Bonus (Sanctum custom) - accumulate positive SC damage for the kill bonus
    if (auto* mob = dynamic_cast<CMobEntity*>(PDefender))
    {
        if (damage > 0)
        {
            mob->addSCMBExpBonusDmg(static_cast<uint32>(damage));
        }
    }
...
void ClaimMob(CBattleEntity* PDefender, CBattleEntity* PAttacker, bool passing)
{
    ...
    if (auto* mob = dynamic_cast<CMobEntity*>(PDefender))
    {
        ...
        if (mob->getMobMod(xi::MobMod::ClaimType) == static_cast<int16>(ClaimType::Unclaimable))
        {
            return;
        }
...
auto GetRuneEnhancementDamageType(EFFECT runeEffect) -> xi::DamageType
{
    switch (runeEffect)
    {
        case EFFECT_IGNIS:
