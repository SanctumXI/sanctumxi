...
local function applyRoll(caster, target, inAbility, action, total, isDoubleup, currentAbility)
    ...
    caster:setLocalVar('corsairApplyingRoll', 1) -- Removed Bust Duration merit. Sanctum Custom
    if not target:addCorsairRoll(caster:getMainJob(), 0, corsairRollMods[abilityId][4], effectpower, 0, duration, corsairRollMods[abilityId][5], total, 0, xi.effectSourceType.CORSAIR_ROLL, caster:getID(), caster:getID()) then
