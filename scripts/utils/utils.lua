...
-- Compatibility helper used by core pet attack and accuracy calculations.
---@nodiscard
...
function utils.getEcosystemStrengthBonus(attackerSystem, defenderSystem)
    if xi.data.entityCorrelation.getEcosystemStrongAgainst(attackerSystem) == defenderSystem then
        return 1
    elseif xi.data.entityCorrelation.getEcosystemWeakAgainst(attackerSystem) == defenderSystem then
        return -1
    end
