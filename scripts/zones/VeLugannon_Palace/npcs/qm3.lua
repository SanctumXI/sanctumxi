...
entity.onTrade = function(player, npc, trade)
    if npcUtil.tradeMatches(trade, xi.item.BUCCANEERS_KNIFE) then -- Buccaneer's Knife
        if npc:getLocalVar('PillarCharged') == 1 and math.randomInt(1, 100) <= 5 then -- 5% chance to obtain on trade.
